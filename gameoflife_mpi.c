...
void filling_runner(char *currentfield, int width, int height) {
    if (((gsizes[0] / 2 + 0) >= (coords[0] * lsizes[0])) && ((gsizes[1] / 2 + 1) >= (coords[1] * lsizes[1]))) {
        if (((gsizes[0] / 2 + 0) < (coords[0] * lsizes[0] + lsizes[0])) &&
            ((gsizes[1] / 2 + 1) < (coords[1] * lsizes[1] + lsizes[1]))) {
            currentfield[calcIndex(width, (gsizes[0] / 2 + 0) - (coords[0] * lsizes[0]) + 1,
                                   (gsizes[1] / 2 + 1) - (coords[1] * lsizes[1]) + 1)] = ALIVE;
            printf("rank %d: first cell gindex %d %d lindex %d %d \n", rank_cart, gsizes[0] / 2 + 0, gsizes[1] / 2 + 1,
                   coords[0] * lsizes[0], coords[1] * lsizes[1]);
        }
    }
    if (((gsizes[0] / 2 + 1) >= (coords[0] * lsizes[0])) && ((gsizes[1] / 2 + 2) >= (coords[1] * lsizes[1]))) {
        if (((gsizes[0] / 2 + 1) < (coords[0] * lsizes[0] + lsizes[0])) &&
            ((gsizes[1] / 2 + 2) < (coords[1] * lsizes[1] + lsizes[1]))) {
            currentfield[calcIndex(width, (gsizes[0] / 2 + 1) - (coords[0] * lsizes[0]) + 1,
                                   (gsizes[1] / 2 + 2) - (coords[1] * lsizes[1]) + 1)] = ALIVE;
            printf("rank %d: second cell gindex %d %d lindex %d %d \n", rank_cart, gsizes[0] / 2 + 1, gsizes[1] / 2 + 2,
                   coords[0] * lsizes[0], coords[1] * lsizes[1]);
        }
    }
    if (((gsizes[0] / 2 + 2) >= (coords[0] * lsizes[0])) && ((gsizes[1] / 2 + 0) >= (coords[1] * lsizes[1]))) {
        if (((gsizes[0] / 2 + 2) < (coords[0] * lsizes[0] + lsizes[0])) &&
            ((gsizes[1] / 2 + 0) < (coords[1] * lsizes[1] + lsizes[1]))) {
            currentfield[calcIndex(width, (gsizes[0] / 2 + 2) - (coords[0] * lsizes[0]) + 1,
                                   (gsizes[1] / 2 + 0) - (coords[1] * lsizes[1]) + 1)] = ALIVE;
            printf("rank %d: third cell gindex %d %d lindex %d %d \n", rank_cart, gsizes[0] / 2 + 2, gsizes[1] / 2 + 0,
                   coords[0] * lsizes[0], coords[1] * lsizes[1]);
        }
    }
...
void apply_periodic_boundaries(char *field, int width, int height) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (neighborank[i] != num_tasks && neighborank[i] != rank_cart) {
          if(i < 2){
            ...
            MPI_Status status[2];
            MPI_Isend(sendcellstb[i], width + 1, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[0]));
            MPI_Irecv(recvcellstb[i], width + 1, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[1]));
            ...
            MPI_Request request[2];
            MPI_Status status[2];
            MPI_Isend(sendcellslr[i-2], height + 1, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[0]));
            MPI_Irecv(recvcellslr[i-2], height + 1, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[1]));
    ...
    for (int i = 0; i < 2; i++) {
        printf("%d h %d numrequest %d recvcells %c sendcells %c \n", rank_cart, i, numrequest1, recvcellstb[i][width],sendcellstb[i][width]);
        printf("%d h %d numrequest %d recvcells %c sendcells %c \n", rank_cart, i, numrequest1, recvcellslr[i][height],sendcellslr[i][height]);
    }
