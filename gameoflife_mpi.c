    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (neighborank[i] != num_tasks && neighborank[i] != rank_cart) {
          if(i < 2){
            ...
            MPI_Isend(&sendcellslr[i-2], height + 2, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[numrequest1]));
            MPI_Irecv(&recvcellslr[i-2], height + 2, MPI_CHAR, neighborank[i], 1, cart_comm, &(request[numrequest1 + 1]));
            numrequest1 = numrequest1 + 2;
